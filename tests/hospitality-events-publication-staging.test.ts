...
describe("staging Process Hospitalité & événements", () => {
  const entries = buildHospitalityEventsPublicationStaging();
  ...
  it.each(entries)("$name est synchronisé avec le référentiel", (entry) => {
    expect(entry.targetProcessCount).toBeGreaterThanOrEqual(9);
