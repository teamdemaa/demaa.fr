...
import { POST } from "@/app/api/reprise-project/route";

function request(overrides: Record<string, unknown> = {}) {
  return new Request("https://demaa.fr/api/reprise-project", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Origin: "https://demaa.fr",
      Referer: "https://demaa.fr/a-reprendre",
    },
    body: JSON.stringify({
      activity: "Maintenance industrielle",
      attribution: { version: 1 },
      budget: "800 000 €",
      email: "repreneur@example.com",
      faxNumber: "",
      idempotencyKey: "reprise:12345678",
      message: "Je recherche une PME avec une équipe autonome.",
      name: "Camille Martin",
...
describe("reprise project route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mocks.resolveLeadContext.mockResolvedValue({
      ...
      sourceUrl: "https://demaa.fr/a-reprendre",
    });
  ...
  it("sends the buyer criteria and contact details by email", async () => {
    ...
    expect(mocks.submitLeadRequest).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        fields: expect.arrayContaining([
          { label: "Activité recherchée", value: "Maintenance industrielle" },
          { label: "Région", value: "Auvergne-Rhône-Alpes" },
          { label: "Budget maximal", value: "800 000 €" },
          {
            label: "Projet du repreneur",
            value: "Je recherche une PME avec une équipe autonome.",
          },
        ]),
      }),
    );
  ...
  it("rejects incomplete requests and silently accepts the honeypot", async () => {
    expect((await POST(request({ activity: "" }))).status).toBe(400);
