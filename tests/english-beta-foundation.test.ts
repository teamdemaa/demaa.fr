...
describe("hidden English beta foundation", () => {
  ...
  it("keeps standalone authentication and Google callbacks in the return locale", () => {
    ...
    expect(sharedConnexion).toContain("getAuthUiCopy(localeCode)");
    expect(sharedConnexion).toContain("choiceTitle={copy.page.signInTitle}");
    ...
    expect(googleCallback).toContain("copy.google.callbackTitle");
    expect(googleCallback).toContain("copy.google.retry");
    expect(authCopy).toContain('callbackTitle: "Signing in with Google"');
    expect(authCopy).toContain('retry: "Try again with Google"');
