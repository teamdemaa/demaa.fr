...
describe("canonical system detail routes", () => {
  it("serves system pages from the public Système métier namespace", async () => {
    await expect(access(path.join(root, "src/app/(french)/(marketing)/systemes/[slug]/page.tsx"))).resolves.toBeUndefined();
    await expect(
      access(path.join(root, "src/app/(french)/(marketing)/systemes/[slug]/recapitulatif/page.tsx")),
    ).resolves.toBeUndefined();
    await expect(
      access(path.join(root, "src/app/(french)/(marketing)/systemes/[slug]/processus/page.tsx")),
    ).resolves.toBeUndefined();
