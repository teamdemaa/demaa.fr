...
describe("system page SEO published Solutions boundary", () => {
  it("describes Process and the universal Resources when the registry is empty", () => {
    ...
    expect(metadata.title).toBe(
      `Système opérationnel ${system.name} : Process et Ressources | Demaa`,
    );
  ...
  it("keeps all 115 empty-registry pages free of historical Models while listing Resources", () => {
    ...
    for (const currentEnterprise of enterpriseCatalog) {
      ...
      expect(metadata.title).toBe(
        `Système opérationnel ${currentData.system.name} : Process et Ressources | Demaa`,
      );
  ...
  it("uses Process, Solutions et Ressources and only renderable published resources", () => {
    ...
    expect(metadata.title).toBe(
      `Système opérationnel ${system.name} : Process, Solutions et Ressources | Demaa`,
    );
    ...
    expect(itemList(jsonLd)?.name).toBe(
      `Process, Solutions et Ressources du système opérationnel ${system.name}`,
    );
