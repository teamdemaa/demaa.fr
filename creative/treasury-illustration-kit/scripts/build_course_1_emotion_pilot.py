#!/usr/bin/env python3
"""Build the emotional Course 1 pilot and its validation boards."""
...
PILOT = ROOT / "output" / "course-1-emotion-pilot"
PNG_DIR = PILOT / "png"
SVG_DIR = PILOT / "svg"
...
ASSETS = [
    ("01_commandes_pleines_cash_vide", "Inquiétude — le paradoxe"),
    ("02_benefice_cash_retard", "Déclic — comprendre le retard"),
    ("03_rituel_lundi_controle", "Soulagement — reprendre le contrôle"),
]
...
def build_green_board(images):
    ...
    draw.text((88, 60), "Cours 1 — arc émotionnel", font=font(46, True), fill=WHITE)
    draw.text(
        (90, 126),
    ...
    output = BOARDS_DIR / "course_1_emotion_pilot_green.png"
...
def build_contrast_board(images):
    ...
    draw.text(
        (74, 112),
        "Inquiétude → compréhension → maîtrise",
        font=font(18),
        fill=MINT,
    )
    ...
    output = BOARDS_DIR / "course_1_emotion_pilot_contrast.png"
    board.save(output, optimize=True)
    return output
...
def main():
    ...
    report = {
        ...
        "version": "course-1-emotion-pilot",
