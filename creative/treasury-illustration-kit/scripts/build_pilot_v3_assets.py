#!/usr/bin/env python3
"""Clean chroma remnants, lock the V3 palette, and build validation assets."""
...
def build_green_board(images):
    ...
    draw.text((88, 60), "Pilote V3 — plus de personnalité", font=font(46, True), fill=WHITE)
    draw.text(
        (90, 126),
    ...
    output = BOARDS_DIR / "pilot_v3_green_background_test.png"
...
def build_contrast_board(images):
    ...
    draw.text(
        (74, 112),
        "Gestes, rythme et asymétrie — sans cartouche derrière les illustrations",
        font=font(18),
        fill=MINT,
    )
    ...
    output = BOARDS_DIR / "pilot_v3_contrast_sheet.png"
    board.save(output, optimize=True)
    return output
