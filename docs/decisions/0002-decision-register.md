...
| D-076 | `validated` | Générateur de plan d'action et Système sélectionné | ADR 0008 + contrat produit D-076 ; implémentation MVP autorisée | `/` reçoit un grand champ libre ; une génération principale produit Actions, quatre piliers et un `systemId` parmi 115 slugs canoniques ; le Système se change sans IA ; mémoire invitée éphémère puis Firebase unique après sauvegarde. Pricing ouvert et Accompagnement différé. |
