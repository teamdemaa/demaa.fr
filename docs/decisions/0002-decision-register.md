...
- Mise à jour d'état : 16 août 2026, nouvelle Stratégie d'entreprise cadrée
...
| D-075 | `validated` | Accompagnement France et contact WhatsApp | ADR 0006 | Neuf accompagnements canoniques, composition métier explicite, cartes unifiées, prix actuels et demande de contact WhatsApp manuelle. |
...
| D-079 | `working` | Plan V4, multi-plans, multi-systèmes, dictée et mesure IA | candidat local, ADR 0011 | V4 génère uniquement Actions + `systemId`, conserve les supports typés déterministes et lit V1/V2/V3/manual sans migration destructive. Les anciens champs Stratégie V3 restent isolés, invisibles, non éditables et non transmis par la commande courante. Ils ne constituent pas le socle de la nouvelle Stratégie D-084. Une identité peut gérer plusieurs plans et chaque plan plusieurs Systèmes. Le microphone est centralisé. Le ledger conserve uniquement des métriques sans contenu. L'envoi de la commande et des actions visibles à Vercel AI Gateway reste autorisé ; notes, identité, situation source, historique et données Système restent exclus. |
...
| D-082 | `validated` | Solutions dans la navigation principale et processus métier imprimables | ADR 0012 | La navigation devient `Plan d’action / Solutions / Académie / Opportunités`. Le Plan affiche directement les Actions. Les URLs historiques sont normalisées vers `view=solutions`. La vue Solutions conserve le sélecteur Système puis `Outils / Accompagnement / Ressources`; la première ressource ouvre une liste imprimable limitée aux processus métier. |
| D-084 | `working` | Pilotage d'entreprise : Chiffres mensuels et Stratégie par cycles | ADR 0013 et programme directeur du 16 août ; périmètre Chiffres restauré, décisions produit fermées, implémentation après gate Plans | Chiffres et la nouvelle Stratégie appartiennent à l'entreprise, jamais à un plan, et ne sont jamais transmis à l'IA. D-082 reste la navigation principale cible ; après sauvegarde d'un plan, la vue Plan expose `Plan d'action / Chiffres / Stratégie` via `section=actions|figures|strategy`. Chiffres suit CA, charges, résultat dérivé et dernière trésorerie par mois avec révisions. Stratégie conserve premier cycle automatique, fenêtre de trois mois calendaires Europe/Paris sans expiration, nouveau cycle vide, pagination par 10, accordéon, rétention, conflits inline et suppression par entreprise. La cible top-level `/strategie` est supersédée. |
