...
| D-077 | `validated` | Plan vierge, Opportunités, navigation et identité applicatives | ADR 0010 | L’application affiche `Plan d’action / Opportunités / Académie` dès l'arrivée ; le Plan porte les sous-onglets légers et sans icône `Actions / Solutions`. Les anciennes URLs `view=system` restent acceptées, tandis que les nouveaux liens Solutions utilisent `view=plan&planTab=solutions` sans modifier les routes publiques `/systemes`. Coaching reste accessible via `Échanger`, le header expose `Connexion` en texte et le plan vierge ne requiert pas d'IA. `Rejoindre Team Demaa` ouvre une modale dans Opportunités avec une expertise principale unique ; les sources canoniques et `lead_requests` sont réutilisées sans stockage parallèle. La session Firebase est réutilisée sans ressaisie et sans expérience `Mon espace` ou `Mes plans`. |
