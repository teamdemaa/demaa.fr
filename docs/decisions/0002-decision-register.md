...
| D-078 | `working` | Entreprises et appartenances | fondation locale non activée | Firebase UID reste l'identité et aucune collection `accounts` n'est créée. Le socle utilise `companies` et `company_memberships`, avec une entreprise sans nom et une appartenance `owner` créées au premier plan authentifié. Les nouveaux plans reçoivent `company_id` et les UID d'audit, tout en conservant temporairement `owner_uid` jusqu'au backfill et aux tests d'isolation. Aucun sélecteur, rôle supplémentaire ou système d'invitation n'est inclus. |
...
| D-080 | `validated` | Identité Firebase unique | ADR 0010 | Le compte e-mail et mot de passe Firebase est principal et Google facultatif. Les deux fournisseurs créent le même cookie de session Firebase natif. L'UID est l'unique clé d'autorisation des plans, conversations et brouillons ; l'e-mail sert uniquement de contact. Aucun système d'accès historique n'est conservé. |
