...
- Mise à jour d'état : 14 août 2026, hiérarchie visuelle, Échanger et Team Demaa unifié
...
| D-076 | `validated` | Générateur de plan d'action, Système et accompagnement | ADR 0008, ADR 0009 + contrat produit D-076 | La promesse exacte est « Qu’est-ce qui freine votre entreprise ? » Une génération principale produit Actions et un `systemId` parmi 115 slugs canoniques ; la Stratégie est temporairement masquée et non générée. `Échanger` offre une première clarification par UID, clôturée manuellement avec la réponse finale. Aucun paiement n'est automatique. Un accompagnement mensuel actif ouvre un avantage non cumulable de 12 % sur les autres prestations directement facturées par Demaa et éligibles. |
