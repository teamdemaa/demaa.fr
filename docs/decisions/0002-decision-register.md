...
| D-076 | `validated` | Générateur de plan d'action, Système et Coaching | ADR 0008, ADR 0009 + contrat produit D-076 | La promesse exacte est « Qu’est-ce qui freine votre entreprise aujourd’hui ? Repartez avec un plan d’action concret. » Une génération principale produit Actions, quatre piliers et un `systemId` parmi 115 slugs canoniques ; le Système se change sans IA ; Firebase devient la source persistante après sauvegarde. Coaching propose Sessions et Messages dans la même application. Les libellés humains emploient `spécialiste`, dont `Écrire à un spécialiste`. Pricing de la génération ouvert. |
