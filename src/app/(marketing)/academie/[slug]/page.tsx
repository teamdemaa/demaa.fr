...
function getAcademyCard(slug: string) {
  return getAcademyPreviewCards().find(({ href }) => href === `/academie/${slug}`);
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const card = getAcademyCard(slug);
  if (!card) return { title: "Méthode introuvable | Académie Demaa" };

  return buildPublicPageMetadata({
    title: `${card.title} | Académie Demaa`,
    description: card.summary,
    path: `/academie/${slug}`,
    type: "article",
    keywords: [card.category, ...card.searchTerms],
    ...(card.image
      ? {
          socialImage: {
            alt: `Illustration de la méthode : ${card.title}`,
            height: 900,
            url: card.image,
            width: 1600,
          },
...
export default async function AcademyMethodPage({ params }: Props) {
  ...
  const canonicalUrl = `${origin}/academie/${slug}`;
  const publishedAt = practice?.publishedAt ?? article?.publishedAt;
  const updatedAt = practice?.updatedAt ?? article?.updatedAt;
  const citations = article?.sources.map(({ href }) => href) ?? [];
  const jsonLd = [
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "Académie", item: `${origin}/academie` },
        { "@type": "ListItem", position: 3, name: card.title, item: canonicalUrl },
      ],
    },
    {
      "@context": "https://schema.org",
      ...
      url: canonicalUrl,
      inLanguage: "fr-FR",
      ...(card.image ? { image: `${origin}${card.image}` } : {}),
      ...
      <main className="min-h-screen min-w-0 bg-background pb-20 text-brand-blue">
        <article className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
          <Link href="/academie" className="inline-flex min-h-11 items-center gap-2 text-sm text-dema-muted transition hover:text-dema-forest">
            <ArrowLeft className="h-4 w-4" aria-hidden="true" /> Retour à l’Académie
          </Link>
            ...
            <div className="min-w-0 space-y-12">
              ...
              {practice ? (
                <section aria-labelledby="academy-method-structure">
                  ...
                  <p className="mb-5 mt-4 text-base leading-8 text-dema-muted">Commencez par ces champs, puis adaptez-les à votre activité.</p>
