...
export async function POST(request: Request) {
  try {
    ...
    const { data: body, response } =
      await readJsonBody<MiniCoursesRequestBody>(request);
    ...
    const normalizedSource = normalizeText(body?.source, 120) || "mini_courses_page";
    ...
    await sendSlackMessage({
      text: "Nouvelle inscription mini-cours Demaa",
      blocks: [
        {
          type: "section",
          text: {
    ...
    console.error("Mini courses subscription error:", error);
