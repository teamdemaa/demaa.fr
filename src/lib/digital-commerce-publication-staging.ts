...
export type DigitalCommercePublicationStagingEntry = {
  slug: string;
  name: string;
...
export function buildDigitalCommercePublicationStaging(): DigitalCommercePublicationStagingEntry[] {
  const currentSteps = processSteps.steps as CurrentStep[];

  return Object.values(digitalCommerceProfiles)
    .map((profile) => {
      const coreDraft = generateDigitalCommerceCoreDraft(profile);
      const draft = generateDigitalCommerceDraft(profile);
      const expectedProcessCount = Object.keys(profile.processIds).length;
