    ...
    slides: [
      {
        ...
        subtitle: "Pour que tout ne repose pas que sur vous",
        footer: SITE_URL,
      },
      {
        id: "audit",
        kind: "process-snapshot",
        title: "1-Faire un audit de l'organisation existante",
        subtitle: "Vous pouvez utiliser cette structure pour commencer",
      ...
      },
      {
        id: "tools",
        kind: "tools-snapshot",
        title: "2-Utiliser les outils adaptés à vos réalités",
        subtitle: "Prenez le temps de comparer",
        footer: `Disponible sur ${SITE_URL}`,
        tools,
      },
      {
        id: "process",
        kind: "checklist",
        title: "3-Mettre en place des processus simples et efficaces",
      ...
      },
      {
        ...
        kind: "cta-snapshot",
        title:
          "On peut échanger 30 minutes, pour discuter de votre organisation et identifier les opportunités d'amélioration",
...
function buildMetaCaption(sector: SocialStudioSector, process?: SocialProcess): string {
  return [
    `Structurer ${getIntroSubject(sector.name)}, ce n'est pas ajouter de la complexité.`,
    "",
    "C'est rendre visibles les bons process, les bons outils et les bonnes responsabilités.",
    ...(process ? [`Exemple ici : ${process.title.toLowerCase()}.`] : []),
    "",
...
function buildLinkedinCaption(sector: SocialStudioSector, process?: SocialProcess): string {
  return [
    `${capitalize(getIntroSubject(sector.name))} ne dépend pas uniquement de la mémoire du dirigeant.`,
    "",
    "L'idée n'est pas de tout rigidifier, mais de clarifier :",
    "- les process prioritaires",
    "- les outils utiles",
    "- les points de contrôle",
    "- les responsabilités",
    "",
    ...(process ? [`Dans cet exemple, on part du process : ${process.title}.`] : []),
    "",
...
function buildCanvaBrief(
  ...
  return [
    ...
    "Sous-titre italique : Pour que tout ne repose pas que sur vous",
    "Footer : www.demaa.fr",
    "",
    "Slide 2",
    "Titre : 1-Faire un audit de l'organisation existante",
    "Sous-titre : Vous pouvez utiliser cette structure pour commencer",
    `Screenshot : aperçu processus ${sector.name}, onglet Processus`,
    "Footer : Disponible sur www.demaa.fr",
    "",
    "Slide 3",
    "Titre : 2-Utiliser les outils adaptés à vos réalités",
    "Sous-titre : Prenez le temps de comparer",
    "Outils à afficher :",
    ...tools.slice(0, 4).map((tool) => `- ${tool.name} (${tool.type}) : ${tool.usage}`),
    "Footer : Disponible sur www.demaa.fr",
    "",
    "Slide 4",
    "Titre : 3-Mettre en place des processus simples et efficaces",
