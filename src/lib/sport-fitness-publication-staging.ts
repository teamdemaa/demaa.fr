...
export type SportFitnessPublicationStagingEntry = {
  slug: string;
  name: string;
...
const patterns: Record<string, RegExp> = {
  "coach-sportif":
    /coach|coaching|individuel|petits groupes|carte professionnelle|prérogatives|client|séance|déplacement|lieu partenaire|ressenti d’effort|répétitions/i,
  "salle-de-sport":
    /salle|EAPS|ERP|Signal-Sports|adhérent|abonnement|SEPA|machine|vestiaire|cours collectif|affluence|résiliation|médiateur|rétractation/i,
};

export function buildSportFitnessPublicationStaging(): SportFitnessPublicationStagingEntry[] {
  const currentSteps = processSteps.steps as CurrentStep[];
  const core = generateSportFitnessCoreDraft();

  return Object.values(sportFitnessProfiles)
    .map((profile) => {
