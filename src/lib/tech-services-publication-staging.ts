...
export type TechServicesPublicationStagingEntry = {
  slug: string;
  name: string;
...
export function buildTechServicesPublicationStaging(): TechServicesPublicationStagingEntry[] {
  const currentSteps = processSteps.steps as CurrentStep[];

  return Object.values(techServicesTradeProfiles)
    .map((profile) => {
      ...
      const explicitTradeDifferences = Object.entries(
        ...
        (total, [processId, items]) =>
          ...
          items.filter(
            (entry, index) =>
              entry.label !==
              techServicesFamilyCoreDraft.contentByProcessId[processId]?.[index]
                ?.label,
          ).length,
        0,
      );
