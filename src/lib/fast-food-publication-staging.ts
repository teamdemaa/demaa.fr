...
export type FastFoodPublicationStagingEntry = {
  slug: string;
  name: string;
...
export function buildFastFoodPublicationStaging(): FastFoodPublicationStagingEntry[] {
  const currentSteps = processSteps.steps as CurrentStep[];

  return Object.values(fastFoodTradeProfiles)
    .map((profile) => {
      ...
      const explicitTradeDifferences = Object.entries(
        ...
        (total, [processId, items]) =>
          ...
          items.filter(
            (entry, index) =>
              entry.label !==
              fastFoodFamilyCoreDraft.contentByProcessId[processId]?.[index]
                ?.label,
          ).length,
        0,
      );
