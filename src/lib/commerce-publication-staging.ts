...
export type CommercePublicationStagingEntry = {
  slug: string;
  name: string;
...
export function buildCommercePublicationStaging(): CommercePublicationStagingEntry[] {
  const currentSteps = processSteps.steps as CurrentStep[];

  return Object.values(commerceTradeProfiles)
    .map((profile) => {
      ...
      const explicitTradeDifferences = Object.entries(
        ...
        (total, [processId, items]) =>
          ...
          items.filter(
            (entry, index) =>
              entry.label !==
              commerceFamilyCoreDraft.contentByProcessId[processId]?.[index]
                ?.label,
          ).length,
        0,
      );
