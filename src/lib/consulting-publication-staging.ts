...
export type ConsultingPublicationStagingEntry = {
  slug: string;
  name: string;
...
export function buildConsultingPublicationStaging(): ConsultingPublicationStagingEntry[] {
  const currentSteps = processSteps.steps as CurrentStep[];

  return Object.values(consultingTradeProfiles)
    .map((profile) => {
      ...
      const explicitTradeDifferences = Object.entries(
        ...
        (total, [processId, items]) =>
          ...
          items.filter(
            (entry, index) =>
              entry.label !==
              consultingFamilyCoreDraft.contentByProcessId[processId]?.[index]
                ?.label,
          ).length,
        0,
      );
