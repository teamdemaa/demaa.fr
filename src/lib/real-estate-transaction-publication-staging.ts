...
export type RealEstateTransactionPublicationStagingEntry = {
  slug: string;
  name: string;
...
const distinctiveTradePatterns: Record<string, RegExp> = {
  "agence-immobiliere":
    /agence|vendeur|bailleur|mandat de vente|mandat de location|bien|annonce|DPE|diagnostic|acquéreur|locataire|visite|offre|compromis|notaire|honoraires|carte professionnelle|registre/i,
  "chasseur-immobilier":
    /chasseur|mandat de recherche|acquéreur|acheteur|financement|cahier des charges|recherche|sourcing|off-market|shortlist|visite|offre|notaire|courtier|budget|rendement/i,
};

export function buildRealEstateTransactionPublicationStaging(): RealEstateTransactionPublicationStagingEntry[] {
  const currentSteps = processSteps.steps as CurrentStep[];
  const coreDraft = generateRealEstateTransactionCoreDraft();

  return Object.values(realEstateTransactionProfiles)
    .map((profile) => {
