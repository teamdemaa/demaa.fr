...
import { Check, HandHeart, LoaderCircle, X } from "lucide-react";
...
type B2BOpportunityInterestModalProps = {
  onClose: () => void;
  opportunity: B2BOpportunity;
};

type InterestPayload = { error?: string } | { ok: true } | null;

function getErrorMessage(response: Response, payload: InterestPayload) {
  ...
  return "Impossible d’enregistrer votre intérêt pour le moment. Réessayez dans quelques instants.";
}

export default function B2BOpportunityInterestModal({
  onClose,
  opportunity,
}: B2BOpportunityInterestModalProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/opportunites-b2b/interest", {
        method: "POST",
      ...
      const payload = (await response.json().catch(() => null)) as InterestPayload;
      ...
      setIsSuccess(true);
      trackLeadConversion({ requestType: "b2b_opportunity_interest" });
    } catch {
      setError(
        "Impossible d’enregistrer votre intérêt pour le moment. Réessayez dans quelques instants.",
      );
    } finally {
  ...
  return (
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="b2b-interest-modal-title"
        aria-describedby="b2b-interest-modal-description"
        tabIndex={-1}
      >
        <button
          type="button"
          ...
          {isSuccess ? (
            ...
            <>
              ...
              <p
                ...
                className="mt-3 text-sm leading-relaxed text-dema-muted"
              >
                Laissez vos coordonnées, l’équipe Demaa fait le lien.
              </p>
              ...
              <form
                ...
                noValidate
              >
                <label className="block text-sm font-medium text-brand-blue" htmlFor="b2b-interest-name">
                  Nom et prénom
                </label>
                <input
                  ref={firstFieldRef}
                  id="b2b-interest-name"
                  name="fullName"
                ...
                <label className="block text-sm font-medium text-brand-blue" htmlFor="b2b-interest-email">
                  Adresse e-mail
                </label>
                <input
                  id="b2b-interest-email"
                  name="email"
                  className="demaa-input"
                  type="email"
                  value={email}
                ...
                <div
                  ...
                  aria-hidden="true"
                >
                  <label htmlFor="b2b-interest-website">Site internet</label>
                  <input
                    id="b2b-interest-website"
                    name="website"
                    type="text"
                    value={website}
                ...
                >
                  ...
                  {isSubmitting ? "Envoi…" : "Envoyer mon intérêt"}
                </button>
              </form>
            </>
          )}
