...
export default function DriveFolderTreePreview({ template }: { template: DriveFolderTemplate }) {
  return (
    <div className="h-full min-h-[25rem] max-h-[40rem] overflow-auto p-5 sm:p-7">
      ...
      <div className="mb-5 rounded-xl bg-dema-sage/45 p-4 text-xs leading-5 text-dema-muted">
        <p>Drive classe les fichiers. Les objectifs, décisions, tâches et réunions se suivent dans le modèle de pilotage.</p>
        <Link
          href="/modeles/pilotage-entreprise-notion"
          className="mt-2 inline-block font-medium text-dema-forest underline decoration-dema-forest/25 underline-offset-4 hover:decoration-dema-forest"
        >
          Voir le modèle de pilotage Notion
        </Link>
      </div>
