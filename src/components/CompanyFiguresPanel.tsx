...
export default function CompanyFiguresPanel({
  ...
  initialEntryPeriod?: CompanyMonth;
  onEntryRequestConsumed?: () => void;
  onAuthenticationRequired?: (period: CompanyMonth) => void;
}) {
  ...
  const [dialogOpen, setDialogOpen] = useState(
    authenticated && Boolean(initialEntryPeriod),
  );
  ...
  function openMetricEntry(period: CompanyMonth) {
    if (!authenticated) {
      onAuthenticationRequired?.(period);
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-[68rem] pb-12">
      ...
      {!loading && periods.length ? (
        <>
          <dl className="mt-6 grid grid-cols-2 gap-3">
            {[['Chiffre d’affaires', summary.revenueCents], ['Charges', summary.expensesCents], ['Résultat de pilotage', summary.resultCents], ['Dernière trésorerie', summary.cashBalanceCents]].map(([label, value]) => <div key={String(label)} className="min-w-0 rounded-2xl border border-dema-line bg-dema-paper p-3 sm:p-4"><dt className="text-xs text-dema-muted sm:text-sm">{label}</dt><dd className="mt-1 break-words text-lg font-semibold text-dema-ink sm:text-xl">{formatCents(value as number | null)}</dd>{label === 'Résultat de pilotage' ? <p className="mt-1 text-[11px] leading-snug text-dema-muted sm:text-xs">Indicateur de pilotage, non comptable.</p> : null}</div>)}
          </dl>
          {periods.length > 1 ? <section className="mt-8 border-t border-dema-line pt-6" aria-labelledby="company-metrics-chart-title">
            <div className="flex flex-wrap items-end justify-between gap-3"><h2 id="company-metrics-chart-title" className="text-lg font-semibold text-dema-ink">Évolution mensuelle</h2><label className="text-sm font-medium text-dema-ink">Comparer<select value={comparison} onChange={(event) => setComparison(event.target.value as Comparison)} className="mt-1 block rounded-xl border border-dema-line bg-white px-3 py-2 text-sm"><option value="revenue-expenses">CA / Charges</option><option value="revenue-cash">CA / Trésorerie</option><option value="result-cash">Résultat / Trésorerie</option></select></label></div>
            <p id="company-metrics-chart-instructions" className="sr-only">Utilisez la touche Tab pour parcourir les mois et entendre leurs valeurs.</p>
      ...
      {authenticated ? <CompanyMetricEntryDialog open={dialogOpen} initialPeriod={selectedPeriod} metricsByPeriod={byPeriod} onClose={closeMetricEntry} onSaved={(metric) => { setMetrics((current) => [...current.filter((item) => item.period !== metric.period), metric].sort((a, b) => a.period.localeCompare(b.period))); }} /> : null}
    </div>
  );
}
