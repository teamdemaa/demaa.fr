...
function AccompanimentDialog({ onClose }: { onClose: () => void }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const flowKey = "accompaniment-request:initial";
    setState("submitting");
    setError("");
    try {
      ...
      form.reset();
      setState("success");
      trackLeadConversion({ requestType: "accompaniment_request" });
...
export default function AccompanimentContactControl({ className = defaultButtonClassName, label = "Prendre rendez-vous" }: { className?: string; label?: string }) {
  const [open, setOpen] = useState(false);
  return <><button type="button" onClick={() => setOpen(true)} className={className}>{label}</button>{open ? createPortal(<AccompanimentDialog onClose={() => setOpen(false)} />, document.body) : null}</>;
}
