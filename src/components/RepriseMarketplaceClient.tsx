...
import { Building2, Check, Clock3, Euro, Map as MapIcon, MapPin, Search, SlidersHorizontal, UsersRound, X } from "lucide-react";
...
function OpportunityCard({ opportunity, isCompared, onCompare, onPreview, onSelect }: CardProps) {
  return (
    <article className="flex h-full min-h-[20rem] flex-col rounded-[1.75rem] border border-dema-line bg-dema-paper p-6 transition hover:border-dema-forest/30 sm:p-7" onMouseEnter={() => onPreview(opportunity.id)} onMouseLeave={() => onPreview(undefined)}>
      ...
      <button type="button" onClick={onCompare} aria-pressed={isCompared} className={`mt-5 self-start border-b text-xs font-medium transition ${isCompared ? "border-dema-forest text-dema-forest" : "border-transparent text-dema-muted hover:border-dema-muted"}`}>{isCompared ? "Retirer du comparatif" : "Ajouter au comparatif"}</button>
    </article>
  );
}

function ComparisonDialog({ opportunities, onClose }: { opportunities: readonly RepriseOpportunity[]; onClose: () => void }) {
  const rows = [
    ["Activité", (item: RepriseOpportunity) => item.activity],
  ...
  return <DirectoryDetailDialogShell ariaLabel="Comparer les entreprises" maxWidthClassName="max-w-6xl" onClose={onClose}><p className="text-xs font-medium uppercase tracking-[0.14em] text-dema-forest">Comparatif</p><h2 className="mt-3 text-3xl font-normal tracking-[-0.04em] sm:text-4xl">Comparer les entreprises.</h2><p className="mt-3 text-sm leading-6 text-dema-muted">Les informations absentes restent indiquées comme non disponibles. Aucun score automatique n’est appliqué.</p><dl className="mt-7 space-y-4">{rows.map(([label, resolve]) => <div key={label} className="rounded-2xl border border-dema-line p-4"><dt className="text-xs font-medium uppercase tracking-[0.1em] text-dema-forest">{label}</dt><dd className={`mt-3 grid gap-3 ${opportunities.length === 3 ? "lg:grid-cols-3" : "sm:grid-cols-2"}`}>{opportunities.map((opportunity) => <span key={opportunity.id} className="rounded-xl bg-dema-cream/55 p-3 text-sm leading-6"><span className="mb-1 block text-xs text-dema-muted">{opportunity.activity}</span>{resolve(opportunity) || "Non disponible"}</span>)}</dd></div>)}</dl></DirectoryDetailDialogShell>;
}
...
export default function RepriseMarketplaceClient({ opportunities }: { opportunities: readonly RepriseOpportunity[] }) {
  ...
  return (
    <>
      ...
      {comparisonOpen ? createPortal(<ComparisonDialog opportunities={comparison} onClose={() => setComparisonOpen(false)} />, document.body) : null}
