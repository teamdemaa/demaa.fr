  ...
  return (
    <Link href={`${baseUrl}/${service.slug}`} className={`block group ${widthClasses}`}>
      <div className="w-full h-full bg-white rounded-2xl border border-gray-100 p-5 flex flex-col justify-between shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 transform group-hover:-translate-y-1 overflow-hidden relative">
        ...
        <div className="absolute top-0 right-0 w-32 h-32 bg-white rounded-full blur-2xl -mt-8 -mr-8 transition-opacity opacity-0 group-hover:opacity-100"></div>

        {/* Improved Trust Badge for Services */}
        {isService && (
          <div className="absolute top-5 right-5 flex items-center space-x-1.5 px-3 py-1 bg-white rounded-full animate-in fade-in zoom-in duration-500">
        ...
        <div>
          <div className="w-11 h-11 rounded-xl bg-neutral-100 flex items-center justify-center mb-4 group-hover:bg-neutral-200 transition-colors">
            <IconComponent className="w-5 h-5 text-neutral-700 group-hover:text-neutral-950 transition-colors" />
          </div>
          <h3 className="text-lg font-bold text-brand-blue group-hover:text-neutral-700 transition-colors leading-tight mb-2">
            {service.name}
          </h3>
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          <span className="px-3 py-1 bg-[#fcfcfc] text-brand-blue/60 text-[11px] font-medium tracking-wide rounded-full">
            {service.category}
          </span>
        </div>
      </div>
