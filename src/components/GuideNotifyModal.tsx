...
export default function GuideNotifyModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[80] flex items-center justify-center bg-brand-blue/35 p-4 backdrop-blur-sm"
      onMouseDown={onClose}
    >
      <div
        ref={dialogRef}
        role="dialog"
        aria-modal="true"
        aria-labelledby="guide-notify-title"
      ...
      >
        ...
        {success ? (
          <div role="status">
            <Check className="h-7 w-7 text-dema-forest" />
            <h2
              id="guide-notify-title"
              className="mt-4 text-2xl font-semibold text-brand-blue"
            >
              Vous serez informé.
            </h2>
            <p className="mt-3 text-sm leading-relaxed text-dema-muted">
              Nous vous écrirons uniquement lorsque « {resource.title} » sera
              disponible.
            </p>
            <button
              type="button"
              onClick={onClose}
              className="demaa-primary-button mt-6 w-full"
            >
              Fermer
            </button>
          </div>
          ...
          <>
            <h2
              id="guide-notify-title"
              className="pr-10 text-2xl font-semibold text-brand-blue"
            >
              Être informé
            </h2>
            <p className="mt-3 text-sm leading-relaxed text-dema-muted">
              Laissez votre e-mail pour recevoir une notification à la sortie de
              ce guide. Ce n’est pas une inscription à une newsletter.
            </p>
            <form className="mt-6 space-y-4" onSubmit={submit}>
              ...
              <p className="text-xs leading-relaxed text-dema-muted">
                Votre adresse sert uniquement à vous prévenir de la sortie de ce
                guide.
              </p>
              {error ? (
              ...
              <button
                disabled={busy}
                className="demaa-primary-button w-full disabled:opacity-60"
              >
                {busy ? "Envoi…" : "M’informer"}
              </button>
            </form>
          </>
        )}
